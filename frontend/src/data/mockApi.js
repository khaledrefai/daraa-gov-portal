...
export async function fetchServicesByCategory(category) {
  await delay();
  return servicesData.filter((service) => service.category === category).map((service) => ({ ...service }));
}

export async function fetchServiceById(id) {
  await delay();
  const service = servicesData.find((item) => item.id === id);
  return service ? { ...service, contacts: service.contacts.map((c) => ({ ...c })), flow: [...service.flow] } : null;
}
...
export async function register({ name, nationalId, phone, password }) {
  await delay();
  ...
  return { message: 'تم إرسال رمز التحقق برسالة نصية', otp };
}
...
export async function submitComplaint({ userPhone, subject, description }) {
  await delay();
  const complaint = {
    id: `c-${Date.now()}`,
    userPhone,
  ...
  return { ...complaint, updates: [...complaint.updates] };
}

export async function listComplaints(phone) {
  await delay();
  return complaints
    .filter((c) => c.userPhone === phone)
    .map((item) => ({ ...item, updates: [...item.updates] }));
}

export async function submitRequest({ userPhone, type, details }) {
  await delay();
  const request = {
    id: `r-${Date.now()}`,
    userPhone,
  ...
  return { ...request, updates: [...request.updates] };
}

export async function listRequests(phone) {
  await delay();
  return requests
    .filter((r) => r.userPhone === phone)
    .map((item) => ({ ...item, updates: [...item.updates] }));
}

export async function listViolations(phone) {
  await delay();
  return violations
    .filter((v) => v.userPhone === phone)
    .map((item) => ({ ...item }));
}
